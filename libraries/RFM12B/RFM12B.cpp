...
uint8_t RFM12B::rssi; // received signal strength
uint8_t RFM12B::rssi_pin; // received signal strength pin
...
void RFM12B::Initialize(uint8_t ID, uint8_t freqBand, uint8_t networkid, uint8_t txPower, uint8_t airKbps, uint8_t lowVoltageThreshold)
{
  ...
  //XFER(0x94C4);             // VDI,FAST,67kHz,0dBm,-79dBm 
...
bool RFM12B::CanSend() {
  ...
  if (rxstate == TXRECV && rxfill == 0 && (Control(0x00) & (RF_RSSI_BIT >> 8)) == 0) {
